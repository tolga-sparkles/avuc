...
function Visualizer({ audioRef, isPlaying }) {
  ...
  const draw = useCallback(() => {
    ...
    // Simulated bars if Web Audio not available, real if possible
    const bars = 32
...
export default function EmergencyRadio({ onToast }) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      {/* Player Card */}
      <div className="rounded-[2rem] border border-avuc-line bg-white p-6 shadow-soft mb-6">
        <div className="flex flex-col items-center gap-5 sm:flex-row sm:justify-between">
          <div className="flex items-center gap-4">
            <div className={classNames('flex h-14 w-14 items-center justify-center rounded-2xl', activeStation ? 'bg-purple-100' : 'bg-slate-100')}>
              {activeStation ? <Radio className="h-7 w-7 text-purple-600" /> : <Headphones className="h-7 w-7 text-slate-400" />}
            </div>
            <div className="min-w-0">
              <h3 className="text-base font-black text-avuc-text">{activeStation ? activeStation.name : 'İstasyon Seçin'}</h3>
              <p className="text-xs text-avuc-muted">{activeStation ? activeStation.desc : 'Yayın başlatmak için kanal seçin'}</p>
              {activeStation && (
        ...
        {error && (
          <div className="mt-4 flex items-center gap-2 rounded-2xl bg-red-50 px-4 py-3 text-sm font-bold text-red-600">
            <AlertTriangle className="h-5 w-5 shrink-0" />
            Yayın yüklenemedi. İnternet bağlantınızı kontrol edin veya başka bir istasyon deneyin.
          </div>
        )}
        ...
        {STATIONS.map((station) => {
          ...
          return (
            <button
              ...
              className={classNames(
                'flex items-center justify-between rounded-2xl border p-4 text-left transition active:scale-[0.98]',
                isActive ? 'border-purple-300 bg-purple-50/60' : 'border-avuc-line bg-white hover:bg-slate-50',
              )}
            >
              <div className="flex items-center gap-3.5">
                <div className={classNames('flex h-10 w-10 items-center justify-center rounded-xl', isActive ? 'bg-purple-500' : 'bg-slate-100')}>
                  {isActive && isPlaying ? <Music className="h-5 w-5 text-white animate-pulse" /> : <Radio className={classNames('h-5 w-5', isActive ? 'text-white' : 'text-slate-400')} />}
                </div>
                <div className="min-w-0">
              ...
              {isActive && isPlaying && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="ml-1.5 text-[10px] font-black text-purple-600 uppercase tracking-wider">Live</span>
                </div>
              )}
            </button>
          )
